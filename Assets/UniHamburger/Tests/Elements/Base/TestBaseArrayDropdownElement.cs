...
namespace nkjzm.UniHamburger.Tests.Elements.Base
{
    public abstract class TestBaseArrayDropdownElement<T> where T : BaseArrayDropdownElement
    {
        ...
        [UnityTest]
        public IEnumerator 初期値が存在しない時は1番目の値が返る()
        {
            return UniTask.ToCoroutine(async () =>
            {
                var token = Utils.CreateTokenToBeCancelled();
        ...
        public IEnumerator uGUIを操作した結果が返る()
        {
            return UniTask.ToCoroutine(async () =>
            {
                var token = Utils.CreateTokenToBeCancelled();
        ...
        public void 要素が1000個ある()
        {
            ...
            for (var i = 0; i < 1000; i++) list.Add(i.ToString());
        ...
        [UnityTest]
        // ReSharper disable once InconsistentNaming
        public IEnumerator 値の更新後に同じ値を選択状態になっている()
        {
            return UniTask.ToCoroutine(async () =>
            {
                var token = Utils.CreateTokenToBeCancelled();
        ...
        [UnityTest]
        public IEnumerator 値の更新後に選択していた値がなくなったら先頭を選択()
        {
            return UniTask.ToCoroutine(async () =>
            {
                var token = Utils.CreateTokenToBeCancelled();
