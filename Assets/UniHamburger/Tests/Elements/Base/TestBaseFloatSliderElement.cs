    ...
    public abstract class TestBaseFloatSliderElement<T> where T : BaseFloatSliderElement
    {
        ...
        public IEnumerator 下限を下回らない()
        {
            return UniTask.ToCoroutine(async () =>
            {
                ...
                var unit = 0.1f;
                // default以外の値を初期値に指定
                var observable = target.Setup("", 1f, unit, 0.9f, salt: DateTime.Now.ToFileTimeUtc().ToString());
                var result = await observable.ToUniTask(true, token);
                Assert.AreEqual(1f, result, unit);
        ...
        public void 普通はたくさん足すと誤差が出る()
        {
            ...
            // たくさん足す
            for (var i = 0; i < 10000; i++) value += unit;
        ...
        [UnityTest]
        public IEnumerator たくさん足しても誤差が出ない()
        {
            return UniTask.ToCoroutine(async () =>
            {
                var token = Utils.CreateTokenToBeCancelled();
                ...
                // たくさん足す
                for (var i = 0; i < 10000; i++) target.AddValue();
